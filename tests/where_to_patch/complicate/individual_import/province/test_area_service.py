...
from python_mock.where_to_patch.complicate.individual_import.province.area_service import (
...
def test_get_province_area_map_patch_client(mocker: MockerFixture) -> None:
    mocker.patch.object(target=area_service_module, attribute="Client")
    get_province_area_map()
...
def test_get_province_area_map(mocker: MockerFixture) -> None:
    mock_population_service_client = mocker.patch.object(target=area_service_module, attribute="Client")
